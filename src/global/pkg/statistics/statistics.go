...
func (s *Statistics) output() {
	...
	for {
		...
		timeIntervalNum++
		// log.NewWarn("", )
		// msg := strings.Join([]string{" system stat ", s.ModuleName, s.PrintArgs, intervalCount, "total:", *s.AllCount, "intervalNum", timeIntervalNum, "avg", (*s.AllCount) / (timeIntervalNum) / s.SleepTime}, "")
	}
}
